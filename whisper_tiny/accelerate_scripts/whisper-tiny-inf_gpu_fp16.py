# whisper-tiny-inf_gpu_fp16.py
# Inferencia en GPU con precisión mixta FP16, batch=16.
# FP16 reduce el ancho de banda de memoria a la mitad respecto a FP32
# y activa los Tensor Cores de la T4. Sin riesgo de underflow en inferencia.
...
def main():
    ...
    # mixed_precision="fp16": Accelerate gestiona el contexto FP16 automáticamente
    ...
    with accelerator.profile() as prof:
        with torch.no_grad():
            for batch_idx, features in enumerate(dataloader):
                ...
                # autocast: operaciones intensivas (GEMM, conv) se ejecutan en float16
                with torch.amp.autocast(device_type=device.type, dtype=torch.float16):
                ...
                print(f"Batch {batch_idx}: {features.size(0)} samples, "
                      f"output[0]={repr(decoded[0])}", flush=True)
                prof.step()
    ...
    print(f"GPU Mem: start={format_bytes(gpu_mem_start)} "
          f"end={format_bytes(gpu_mem_end)} "
          f"peak~={format_bytes(gpu_mem_peak)}", flush=True)
    ...
    if events:
        ...
        summary_time_path = os.path.join(_RESULTS_DIR, f"summary_time_{config_name}.csv")
