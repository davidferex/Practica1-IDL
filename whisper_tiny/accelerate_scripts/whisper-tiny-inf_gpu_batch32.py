# whisper-tiny-inf_gpu_batch32.py
# Inferencia en GPU, FP32, batch=32.
# Estudia el efecto del tamaño de batch en throughput y VRAM respecto al batch=1.
...
def main():
    ...
    print(f"Config: {config_name}  batch_size=32  device={device}", flush=True)
    ...
    batch_size = 32
    num_batches = 8  # 256 total samples
        ...
        with torch.no_grad():
            for batch_idx, features in enumerate(dataloader):
                ...
                print(f"Batch {batch_idx}: {features.size(0)} samples, "
                      f"GPU={gpu_mem/1024**2:.1f} MB", flush=True)
                prof.step()
    ...
    if events:
        ...
        summary_path = os.path.join(_RESULTS_DIR, f"summary_time_{config_name}.csv")
        with open(summary_path, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=time_data[0].keys())
            writer.writeheader()
            writer.writerows(time_data)
        print(f"Resumen del profiling guardado en: {summary_path}", flush=True)
