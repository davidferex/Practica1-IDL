# whisper-tiny-inf_distributed.py
# Inferencia distribuida FP32 en 2 GPUs con Accelerate (DDP), batch=16 por proceso.
# Cada proceso maneja su shard del dataset (DistributedSampler).
# Los resultados se agregan con gather_for_metrics(); solo el proceso principal escribe ficheros.
...
def main():
    ...
    # Accelerate gestiona la coordinación multi-proceso; cada proceso obtiene una GPU
    ...
    # Modelo y procesador
    # Cada proceso carga el modelo independientemente; prepare() lo mueve a su GPU y lo envuelve en DDP
    model = WhisperForConditionalGeneration.from_pretrained("openai/whisper-tiny")
    processor = WhisperProcessor.from_pretrained("openai/whisper-tiny")

    batch_size = 16   # batch por proceso
    # 256 muestras globales repartidas entre los procesos
    num_batches = 256 // (batch_size * accelerator.num_processes)

    # prepare(dataloader) añade DistributedSampler para que cada proceso vea su shard
    ...
    # prepare(model) mueve el modelo a la GPU de este proceso y lo envuelve en DDP
    ...
    with accelerator.profile() as prof:
        with torch.no_grad():
            for batch_idx, features in enumerate(dataloader):
                # features ya está en la GPU de este proceso (prepare() lo movió)
                generated_ids = raw_model.generate(features, max_new_tokens=25)

                # gather_for_metrics recoge los ids de todos los procesos en el principal
                gathered_ids = accelerator.gather_for_metrics(generated_ids)

                # Solo el proceso principal decodifica y registra el batch completo
                if accelerator.is_main_process:
    ...
    # Muestras globales = muestras por proceso × num_procesos
