#!/usr/bin/env python
# distilbert_train_cpu_batch32_grad_acc.py
# Entrenamiento de DistilBERT en CPU usando gradient accumulation, batch de 32 (batch efectivo = 128),
...
# -------------------------------
# Trace handler (para generar trazas)
# -------------------------------
def trace_handler(p):
    output = p.key_averages().table(sort_by="self_cpu_time_total", row_limit=10)
    print(output)

    trace_path = os.path.join(results_dir, f"trace_{p.step_num}_cpu_batch32_gradacc.json")
...
# -------------------------------
config_name = "cpu_batch32_gradacc"
